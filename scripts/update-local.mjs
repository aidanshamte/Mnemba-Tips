...
const origin=await localOrigin(),group=process.argv[2]??'fixtures';
const groups={media:['media','media-news'],'media-news':['media-news'],bootstrap:['catalog','fixtures','results','reconcile','discover','history','entities','identities','archive','backfill','news','social','snapshots'],fixtures:['fixtures','results','snapshots','models'],hourly:['news','social','documents'],daily:['backup','discover','history','reconcile','entities','identities','backfill','mirrors','archive','calibration','retention','search-index','models','rosters'],history:['history'],discover:['discover'],backup:['backup'],reconcile:['reconcile'],news:['news'],social:['social'],entities:['entities']};
...
let failures=0;for(const job of groups[group]){try{if(job==='backup'){log({job,result:await backupLocal()});continue;}const response=await fetch(origin+'/api/intelligence',{method:'POST',headers:{...internalHeaders(),'Content-Type':'application/json'},body:JSON.stringify({job}),signal:AbortSignal.timeout(540000)});const result=await response.json();log({job,status:response.status,result});if(!response.ok||['partial','empty','error'].includes(result.outcome)||result.errors?.length||result.files?.some(r=>r.error||r.fixtures===0))failures++;}catch(e){failures++;log({job,error:e.message});}}process.exitCode=failures?1:0;
