...
export class SourceHttp {
 ...
 async request(id,raw,options={}){
  ...
  for(let attempt=0;attempt<3;attempt++){
   const day=s.day();const reserved=await s.one('INSERT INTO source_request_usage(source_id,day,used) VALUES(?,?,1) ON CONFLICT(source_id,day) DO UPDATE SET used=source_request_usage.used+1 WHERE source_request_usage.used<? RETURNING used',id,day,source.dailyLimit);
