...
export async function savedEstimate(service,fixture,history){
 ...
 const state=matchState(fixture,asOf);if(state.forecastEligible)await new ModelLab(service).capture(fixture);
 ...
 const cached=await s.one('SELECT payload,as_of FROM exploratory_estimates WHERE fixture_id=? AND revision=? AND as_of>? ORDER BY as_of DESC LIMIT 1',fixture.id,revision,asOf-1800000);
