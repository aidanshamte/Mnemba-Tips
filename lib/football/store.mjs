import {mediaMigration} from './schema-v9.mjs';
...
export class Store {
  constructor(db, now = Date.now) { this.db = db; this.now = now; this.readOnly = false; }
  async init() { await this.db.batch([...migration,...intelligenceMigration,...searchMigration,...forecastMigration,...estimateMigration,...lifecycleMigration,...recommendationMigration,...cronMigration,...mediaMigration].map(sql => this.db.prepare(sql))); }
  ...
  async run(sql, ...args) { if (this.readOnly) throw new Error("Public reads cannot mutate storage"); return this.db.prepare(sql).bind(...args).run(); }
  ...
  async putCache(id, data, ttl) { if (this.readOnly) return; await this.upsert('football_cache', { id, payload: JSON.stringify(data), updated_at: this.now(), expires_at: this.now() + ttl }); }
