...
export class FootballService {
  ...
  async predict(f) {
    ...
    prediction.lineupEvidence=confirmedRow?'Confirmed':'PitchPredict model projection';
  ...
  async historical() {
    ...
    try {
      const response=await this.fetcher('https://api.github.com/repos/openfootball/football.json/git/trees/master?recursive=1',{headers:{'User-Agent':'PitchPredict-local'},signal:AbortSignal.timeout(15000),redirect:'manual'});
  ...
  async read(params) {
    ...
    if(view==='catalog') {
      ...
      const competitions=(await s.all('SELECT payload FROM competitions ORDER BY name')).map(r=>JSON.parse(r.payload));
      const seasons=(await s.all('SELECT payload FROM seasons ORDER BY year DESC')).map(r=>JSON.parse(r.payload));
      return {countries,competitions,seasons};
    }
    if(view==='news') {
      const query=`%${(params.get('q')??'').slice(0,200)}%`, category=params.get('category')??'';
      return {articles:deduplicate((await s.all('SELECT payload FROM news_articles WHERE (headline LIKE ? OR payload LIKE ?) AND (?=\'\' OR category=?) ORDER BY published_at DESC LIMIT 300',query,query,category,category)).map(r=>JSON.parse(r.payload))),freshness:await s.cache('job:news')};
    }
    if(view==='search') {
    ...
    const clauses=[],args=[];
    for(const [param,column] of [['competition','f.competition_id'],['season','f.season_id'],['status','f.status'],['country','c.country_id']]) if(params.get(param)) { clauses.push(`${column}=?`); args.push(params.get(param)); }
    ...
    for(const [param,op] of [['from','>='],['to','<=']]) if(params.get(param)) {const date=Date.parse(params.get(param)+(param==='to'?'T23:59:59.999Z':'T00:00:00Z'));if(!Number.isFinite(date))throw new Error('Invalid date');clauses.push(`f.starts_at${op}?`);args.push(date);}
