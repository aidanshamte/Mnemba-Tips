...
export function predictedLineup(players) {
  ...
  return { label: lineup.length === 11 ? 'Mnemba Tips model estimate' : 'Unavailable', formation: '4-3-3', players: lineup.length === 11 ? lineup : [], reason: lineup.length === 11 ? 'Ranked from recorded minutes, performance and availability' : 'Insufficient position, minutes or performance coverage for a full XI' };
}
export function project(fixture, history, context = {}) {
  ...
  return { label: 'Mnemba Tips model estimate', modelVersion: 'football-elo-poisson-1', probabilities: probabilities.map(p=>p/total), expectedGoals: [hx,ax], factors, missing: Object.entries(factors).filter(([,v])=>v===null).map(([k])=>k), evidence: { home,away,h2h }, note: 'Uncalibrated baseline; missing factors are omitted. Provider predictions are separate.' };
}
